...
export default function Comprar() {
  ...
  const preco = sexo === "F" ? (lote?.preco_f ?? 35) : (lote?.preco_m ?? 45);
    ...
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-4 py-20">
      <div className="w-full max-w-md flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-2">
          <p className="text-xs tracking-widest uppercase text-zinc-500">Selecione</p>
          <div className="grid grid-cols-2 gap-3">
            {(["F", "M"] as const).map((s) => {
              const p = s === "F" ? lote?.preco_f : lote?.preco_m;
              return (
                <button
                  key={s}
                  type="button"
                ...
                >
                  ...
                  {p ? <span className="block text-xs mt-1 font-light">R$ {p},00</span> : null}
                </button>
              );
            })}
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <label className="flex items-start gap-3 cursor-pointer group">
            ...
            <div>
              ...
              <p className="text-zinc-500 text-xs mt-0.5">
                + R$ 11,90 por ingresso · Garante reembolso total se desistir
              </p>
            </div>
          </label>
          ...
          <div className="border-t border-zinc-900 pt-4 flex flex-col gap-2">
            <div className="flex justify-between text-sm text-zinc-500">
              <span>{quantidade}x R$ {preco},00</span>
